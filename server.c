	...
	while (1) {
		...
		*/
		...
		while(1)
		{
			...
			// use getline to read the message from the user
			// and store it in temp_buf
			fgets(temp_buf, 5000, stdin);
			
			my_send(newsockfd, temp_buf, strlen(temp_buf), 0);
			...
			can do this.
			*/ 
			int len=my_recv(newsockfd, temp_buf, 5000, 0);
